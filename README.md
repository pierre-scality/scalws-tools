...
The usage cycle is :
1. Create you environement file defining region and machine aws ressources (Usually once)
2. Create a template to AWS with the appropriate AMI for Artesca version
3. Use labws.py build to create the machine(s) 
4. Use labws.py configure to configure the machine(s) (see configure below)

The machines name will be <prefix>-<pattern>-<number> (see build below) and you can create multiple machines with -c option
When you do not need the machine(s) anymore, you can use labws.py destroy to destroy them (with prefix/pattern)
